...
async function getWithRetry(url, options = {}, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const wait = Math.min(retryAfter > 0 ? retryAfter : 2 ** attempt, 30);
...
async function fetchImageFillUrls(fileKey) {
  try {
    const resp = await getWithRetry(
...
async function getFigmaNodeFull(fileKey, nodeId) {
  ...
  for (const nid of nodeIds.slice(0, 3)) {
    ...
    if (imageRefs.size > 0) {
      ...
      imageFillUrls = Object.fromEntries(
        [...imageRefs].map(ref => [ref, allCdnUrls[ref]]).filter(([, url]) => url)
      );
    }

    const result = convertFigmaNode(nodeData, markupType, imageFillUrls);
