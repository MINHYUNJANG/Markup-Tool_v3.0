...
async function getFigmaNodeData(fileKey, nodeId) {
  const resp = await getWithRetry(
    ...
    { headers: figmaHeaders(), signal: AbortSignal.timeout(2000) },
    1
  );
...
규칙: 타이틀 <h2 class="tit1">/<h3 class="tit2">/<h4 class="tit3">, 하위내용 <div class="indent">, 텍스트 <p>, 리스트 <ul class="list_st1">, 순서리스트 <ol class="list_ol1"> 숫자 <span class="num">, 테이블 <div class="tbl_st scroll_gr"><table>, 탭 들여쓰기. HTML만 출력.`;
  ...
  if (!cerebrasResp.ok) {
    const err = await cerebrasResp.json().catch(() => ({}));
    throw new Error(err?.error?.message || `Cerebras API 오류: ${cerebrasResp.status}`);
  }
  const cerebrasData = await cerebrasResp.json();
  return { html: cerebrasData.choices[0].message.content.trim() };
}
