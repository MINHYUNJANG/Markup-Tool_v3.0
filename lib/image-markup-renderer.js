...
function renderSection(section) {
  const pfx = `MC_box${section.idx}`;
  const ext = section.idx === 1 && section.type === 'main-visual' ? ' main-visual' : '';
...
function buildCss(section) {
  ...
  // 섹션 wrapper: bg, padding, flex(quick-menu 제외)
  const sRules = [];
  ...
  (section.elements || []).forEach(el => {
    ...
    if (rules.length) css += `.${pfx}-${suffix} {\n${rules.map(r => `\t${r};`).join('\n')}\n}\n`;
  });
...
export function generateHtmlCss(layoutJson) {
  ...
  const html = sections.map(renderSection).filter(Boolean).join('\n\n');
  const css = sections.map(buildCss).filter(Boolean).join('\n');
  return { html, css };
}
